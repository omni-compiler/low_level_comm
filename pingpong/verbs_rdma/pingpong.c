...
int main(int argc, char **argv)
{
  ...
  MPI_Sendrecv(&(port_attr.lid), sizeof(uint16_t), MPI_BYTE, target, 0,
  ...
  MPI_Sendrecv(&(comm_params->src_psn), sizeof(uint32_t), MPI_BYTE, target, 1,
	       &(comm_params->dest_psn), sizeof(uint32_t), MPI_BYTE, target, 1,
	       MPI_COMM_WORLD, MPI_STATUS_IGNORE);

  MPI_Sendrecv(&(comm_params->qp->qp_num), sizeof(uint32_t), MPI_BYTE, target, 2,
	       &(comm_params->dest_qp_num), sizeof(uint32_t), MPI_BYTE, target, 2,
	       MPI_COMM_WORLD, MPI_STATUS_IGNORE);

  MPI_Sendrecv(&(comm_params->r_mr->rkey), sizeof(uint32_t), MPI_BYTE, target, 3,
	       &(comm_params->dest_rkey), sizeof(uint32_t), MPI_BYTE, target, 3,
	       MPI_COMM_WORLD, MPI_STATUS_IGNORE);

  comm_params->s_src_addr = (uint64_t)(s_buf);
  comm_params->r_src_addr = (uint64_t)(r_buf);
  MPI_Sendrecv(&(comm_params->r_src_addr), sizeof(uint64_t), MPI_BYTE, target, 4,
	       &(comm_params->dest_addr), sizeof(uint64_t), MPI_BYTE, target, 4,
	       MPI_COMM_WORLD, MPI_STATUS_IGNORE);
