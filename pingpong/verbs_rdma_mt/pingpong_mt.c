...
static void post_recv(struct ibv_comm_params *comm_params);
...
#pragma omp parallel shared(time, rank, nprocs, target, hostname) firstprivate(required, provided, num_loop) \
  private(count, iter, ret, s_buf, r_buf)
  {
    ...
    MPI_CHECK(MPI_Sendrecv(&(port_attr.lid), sizeof(uint16_t), MPI_BYTE, target, tag,
			   &(comm_params->dest_lid), sizeof(uint16_t), MPI_BYTE, target, tag,
    ...
    MPI_CHECK(MPI_Sendrecv(&(comm_params->src_psn), sizeof(uint32_t), MPI_BYTE, target, tag+1,
			   &(comm_params->dest_psn), sizeof(uint32_t), MPI_BYTE, target, tag+1,
			   MPI_COMM_WORLD, MPI_STATUS_IGNORE));

    MPI_CHECK(MPI_Sendrecv(&(comm_params->qp->qp_num), sizeof(uint32_t), MPI_BYTE, target, tag+2,
			   &(comm_params->dest_qp_num), sizeof(uint32_t), MPI_BYTE, target, tag+2,
			   MPI_COMM_WORLD, MPI_STATUS_IGNORE));

    MPI_CHECK(MPI_Sendrecv(&(comm_params->r_mr->rkey), sizeof(uint32_t), MPI_BYTE, target, tag+3,
			   &(comm_params->dest_rkey), sizeof(uint32_t), MPI_BYTE, target, tag+3,
			   MPI_COMM_WORLD, MPI_STATUS_IGNORE));

    comm_params->s_src_addr = (uint64_t)(s_buf);
    comm_params->r_src_addr = (uint64_t)(r_buf);
    MPI_CHECK(MPI_Sendrecv(&(comm_params->r_src_addr), sizeof(uint64_t), MPI_BYTE, target, tag+4,
			   &(comm_params->dest_addr), sizeof(uint64_t), MPI_BYTE, target, tag+4,
			   MPI_COMM_WORLD, MPI_STATUS_IGNORE));
...
static void post_send(struct ibv_comm_params *comm_params, size_t length)
{
  ...
  uint64_t dest_addr = comm_params->dest_addr;
  uint32_t rkey = comm_params->dest_rkey;
  ...
  send_wr->imm_data = rank+1;
  send_wr->wr.rdma.remote_addr = dest_addr;
